    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-white dark:bg-navy-950 transition-colors duration-500 overflow-hidden">
      ...
      <nav className="w-full md:w-20 bg-slate-50 dark:bg-navy-900 border-b md:border-b-0 md:border-r border-slate-200 dark:border-slate-800 flex flex-row md:flex-col items-center py-2 md:py-6 px-4 md:px-0 z-50 sticky top-0 md:h-screen shrink-0 shadow-sm md:shadow-none">
        ...
        <div className="flex flex-row md:flex-col items-center gap-1 md:gap-3 flex-1 md:flex-none md:w-full justify-around md:justify-start px-2 md:px-0">
          <button
            onClick={() => setActiveTab(AppTab.QURAN)}
            className={`p-2.5 md:p-3 rounded-xl transition-all relative group ${
              activeTab === AppTab.QURAN 
                ? 'bg-gold text-white shadow-lg scale-110 md:scale-110' 
          ...
          <button 
            ...
            className={`p-2.5 md:p-3 rounded-xl transition-all relative group ${
              activeTab === AppTab.QURAN_AI 
                ? 'bg-navy-800 dark:bg-gold text-white dark:text-navy-950 shadow-lg scale-110 md:scale-110' 
                : 'text-slate-400 hover:text-gold hover:bg-gold/5'
            }`}
          >
          ...
          <button 
            ...
            className="p-2.5 md:p-3 flex items-center justify-center rounded-xl bg-slate-200/50 dark:bg-navy-800/50 text-slate-500 dark:text-gold hover:scale-110 transition-all shadow-sm"
          >
            {isDark ? (
              <svg className="w-4 h-4 md:w-5 md:h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 3v1m0 16v1m9-9h1M4 12H3m15.364 6.364l-.707-.707M6.343 6.344l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" /></svg>
            ) : (
      ...
      <main className="flex-1 flex flex-col h-[100dvh] overflow-hidden">
        <header className="h-14 md:h-16 flex items-center justify-between px-4 md:px-6 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-navy-950/80 backdrop-blur-md shrink-0 z-40">
          <div className="flex items-center gap-2 md:gap-4 overflow-hidden">
             <span className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.15em] md:tracking-[0.2em] text-gold cursor-pointer shrink-0" onClick={() => setActiveTab(AppTab.QURAN)}>Al-Malik</span>
             <div className="w-1 h-1 bg-slate-300 dark:bg-slate-700 rounded-full shrink-0"></div>
             <h1 className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] md:tracking-[0.3em] text-slate-400 truncate">
          ...
          <div className="flex items-center gap-2 md:gap-4 shrink-0">
            <button 
              ...
              className={`text-[9px] md:text-[10px] font-black uppercase tracking-[0.15em] md:tracking-[0.2em] transition-colors ${activeTab === AppTab.DEVELOPER ? 'text-gold' : 'text-slate-500 hover:text-gold'}`}
            >
              SMA
            </button>
            ...
            <button 
              ...
              className={`text-[9px] md:text-[10px] font-black uppercase tracking-[0.15em] md:tracking-[0.2em] transition-colors ${activeTab === AppTab.ABOUT ? 'text-gold' : 'text-slate-500 hover:text-gold'}`}
            >
              About
            </button>
          </div>
